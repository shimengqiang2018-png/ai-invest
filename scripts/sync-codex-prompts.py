#!/usr/bin/env python3
"""Generate Codex custom prompts from AI Invest skills.
...
def prompt_for(source: Path) -> str:
    ...
    description = f"AI Invest slash entry for {title}."
    return (
        "---\n"
        f"description: {yaml_quote(description)}\n"
        "argument-hint: $ARGUMENTS\n"
        "---\n\n"
        f"Use the installed AI Invest Codex skill `{name}` for this request.\n\n"
        f"If the skill is not already loaded, read and follow "
        f"`~/ai-invest/codex-skills/{name}/SKILL.md`.\n\n"
        "User arguments:\n"
        "$ARGUMENTS\n"
    )
