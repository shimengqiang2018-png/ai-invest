#!/usr/bin/env python3
"""Generate Codex skills from AI Invest Claude command files."""
...
def metadata_for(name: str, source_name: str, source_text: str) -> str:
    ...
    if existing:
        ...
        if not has_description:
            ...
            lines.append(
                "description: "
                + yaml_quote(f"AI Invest skill: {title}. Source: skills/{source_name}.")
            )
        lines.append(existing.rstrip())
        return "---\n" + "\n".join(lines) + "\n---\n\n"

    title = first_heading(source_text, name)
    description = f"AI Invest skill: {title}. Source: skills/{source_name}."
    return (
        "---\n"
        f"name: {name}\n"
